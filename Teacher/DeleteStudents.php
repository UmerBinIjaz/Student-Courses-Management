...
if(isset($_GET['id']) && isset($_GET['courseId']) && isset($_GET['shiftId']) && isset($_GET['studentId'])) {
    $teacherId = $_GET['id'];
    ...
    $deleteQuery = "DELETE FROM tblstudent_course_shift WHERE student_id = $studentId AND course_id = $courseId AND shift_id = $shiftId";

    // Perform deletion
    if(mysqli_query($conn, $deleteQuery)) {
        // Student deleted successfully, redirect to CreateStudents.php
        header("Location: AddStudents.php?id=$teacherId&courseId=$courseId&shiftId=$shiftId");
        exit();
    } else {
